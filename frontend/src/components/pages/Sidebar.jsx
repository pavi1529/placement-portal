...
export default function Sidebar({ activeTab, setActiveTab, onLogout }) {
  ...
  const getActiveGradient = (id) => {
    // All active buttons now use white
    return 'linear-gradient(135deg, #ffffff, #f0f0f0)';
  };

  // ✅ Glow colors for active state - white glow
  const getGlowColor = (id) => {
    return 'rgba(255,255,255,0.4)';
  };

  // ✅ Get tab icon with color
  ...
  return (
    <aside className="col-md-3 col-lg-2 position-fixed h-100 shadow-2xl" 
           style={{ 
             top: 0, 
             left: 0, 
             zIndex: 1000, 
      ...
      <div className="px-3 position-relative" style={{ 
        ...
        <nav className="nav flex-column gap-1">
          {navItems.map((tab) => {
            ...
            const gradient = getActiveGradient(tab.id);
            const glowColor = getGlowColor(tab.id);
            ...
            return (
              <button
                ...
                style={{ 
                  ...
                  fontWeight: '500', 
                  border: 'none',
                  background: isActive ? gradient : 'rgba(255,255,255,0.04)',
                  boxShadow: isActive ? `0 4px 20px ${glowColor}` : 'none',
