      ...
      >
        <div className="card-body p-3">
          <div className="d-flex flex-wrap gap-3 align-items-center">
            <div className="flex-grow-1 d-flex align-items-center gap-2 px-3 py-2 rounded-3" style={{ 
              background: 'rgba(0,0,0,0.03)',
              border: '1px solid #e9ecef',
              transition: 'all 0.3s ease'
            }}>
              <Search className="text-secondary" style={{ width: '0.9rem', height: '0.9rem' }} />
              <input 
                type="text" 
                ...
                >
                  <div className="card-body p-4">
                    <div className="d-flex gap-3">
                      ...
                      >
                        <Icon className={`text-${notif.color}`} style={{ width: '1.2rem', height: '1.2rem' }} />
                      </div>
                        ...
                        <div className="d-flex flex-wrap justify-content-between align-items-start gap-2">
                          <div>
                            <div className="d-flex align-items-center gap-2">
                              <span className={`fw-bold text-${notif.color}`} style={{ fontSize: '0.7rem' }}>
                                {notif.type}
                              </span>
                              {!notif.read && (
                                <span className="badge bg-danger bg-opacity-10 text-danger border border-danger" style={{ fontSize: '0.45rem', padding: '0.15rem 0.5rem' }}>
                                  New
                                </span>
                              )}
                              <span className={`badge bg-${getPriorityColor(notif.priority)} bg-opacity-10 text-${getPriorityColor(notif.priority)} border border-${getPriorityColor(notif.priority)}`} style={{ fontSize: '0.45rem', padding: '0.15rem 0.5rem' }}>
                                {notif.priority}
                              </span>
                            </div>
                            <p className="text-dark small mb-1" style={{ fontSize: '0.8rem' }}>
                              {notif.msg}
                            </p>
                          </div>
                          <div className="d-flex flex-column align-items-end gap-1">
                            <span className="text-secondary" style={{ fontSize: '0.55rem' }}>
                              <Clock className="me-1" style={{ width: '0.6rem', height: '0.6rem' }} />
                              {notif.time}
                            </span>
                            {!notif.read && (
                              <span className="d-inline-block rounded-circle bg-primary" style={{ width: '6px', height: '6px' }}></span>
                            )}
                          </div>
                        </div>
                      </div>
